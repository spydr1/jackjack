...
def drop_path(x, drop_prob: float = 0.):
    ...
    random_tensor = keep_prob + keras.random.normal(shape, dtype=tf.float64)
    random_tensor = keras.ops.floor(random_tensor)
    # random_tensor.floor_()  # binarize
    output = keras.ops.divide(x, keep_prob) * random_tensor
    output = keras.ops.cast(output, dtype=x.dtype)
    return output
...
def window_partition_layer(x, window_size):
    ...
    """
    B, H, W, C = x.shape
    x = keras.layers.Reshape([H // window_size, window_size, W // window_size, window_size, C])(x)
    windows = keras.ops.transpose(x, axes=(0, 1, 3, 2, 4, 5))
    windows = keras.ops.reshape(windows, [-1, window_size, window_size, C])
    return windows
...
def window_reverse(windows, window_size, H, W):
    ...
    Returns:
        x: (B, H, W, C)
    """
    B, hh, hw, C = windows.shape

    x = keras.ops.reshape(windows, [-1, H // window_size, W // window_size, window_size, window_size, C])
    x = keras.ops.transpose(x, axes=(0, 1, 3, 2, 4, 5))
...
class WindowAttention(keras.layers.Layer):
    ...
    def call(self, x, mask=None):
        ...
        qkv = self.qkv(x)
        qkv = keras.layers.Reshape([N, 3, self.num_heads, C // self.num_heads])(qkv)
        qkv = keras.ops.transpose(qkv, axes=[2, 0, 3, 1, 4])
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

        q = q * self.scale
        k = keras.ops.swapaxes(k, -2, -1)
        attn = keras.ops.matmul(q, k)

        idx = keras.ops.reshape(self.relative_position_index, [-1])

        relative_position_bias = keras.ops.reshape(tf.gather(self.relative_position_bias_table, idx),
                                                   [self.window_size[0] * self.window_size[1],
                                                    self.window_size[0] * self.window_size[1], -1])  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = keras.ops.transpose(relative_position_bias, axes=[2, 0, 1])  # nH, Wh*Ww, Wh*Ww
        attn = attn + relative_position_bias[None]

        if mask is not None:
            nW = mask.shape[0]
            attn = keras.ops.reshape(attn, [-1, nW, self.num_heads, N, N]) + mask[:, None][None]
            attn = keras.ops.reshape(attn, [-1, self.num_heads, N, N])
...
class RDG(keras.layers.Layer):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, shift_size, mlp_ratio, qkv_bias, qk_scale,
        ...
        self.lrelu = keras.layers.Activation(keras.activations.leaky_relu)
    ...
    def call(self, x, x_size=None, training=False):
        ...
        x2 = self.pe(self.lrelu(self.adjust2(self.pue(self.swin2(keras.ops.concatenate((x, x1), -1), x_size=x_size, training=training), x_size=x_size))))
        x3 = self.pe(self.lrelu(self.adjust3(self.pue(self.swin3(keras.ops.concatenate((x, x1, x2), -1), x_size=x_size, training=training), x_size=x_size))))
        x4 = self.pe(self.lrelu(self.adjust4(self.pue(self.swin4(keras.ops.concatenate((x, x1, x2, x3), -1), x_size=x_size, training=training), x_size=x_size))))
        x5 = self.pe(self.adjust5(self.pue(self.swin5(keras.ops.concatenate((x, x1, x2, x3, x4), -1), x_size=x_size, training=training), x_size=x_size)))
...
class SwinTransformerBlock(keras.layers.Layer):
    ...
    def call(self, x, x_size, training=False):
        ...
        x_windows = keras.ops.reshape(x_windows, [-1, self.window_size * self.window_size, C])  # nW*B, window_size*window_size, C
        ...
        # merge windows
        attn_windows = keras.ops.reshape(attn_windows, [-1, self.window_size, self.window_size, C])
        shifted_x = window_reverse(attn_windows, self.window_size, H, W)  # B H' W' C

        # reverse cyclic shift
        if self.shift_size > 0:
            x = keras.ops.roll(shifted_x, shift=(self.shift_size, self.shift_size), axis=(1, 2))
        else:
            x = shifted_x
        x = keras.layers.Reshape([H * W, C])(x)
...
class PatchMerging(keras.layers.Layer):
    ...
    def call(self, x):
        ...
        x = keras.ops.concatenate([x0, x1, x2, x3], -1)  # b h/2 w/2 4*c
        x = keras.ops.reshape(x, [b, -1, 4 * c])  # b h/2*w/2 4*c
...
class PatchUnEmbed(keras.layers.Layer):
    ...
    def call(self, x, x_size):
        ...
        # x = keras.ops.swapaxes(x, 1,2)
        x = keras.layers.Reshape([x_size[0], x_size[1], C])(x)
        return x
...
class PixelShuffle(keras.layers.Layer):
    ...
    def call(self, x):
        ...
        x = keras.layers.Reshape([hh, hw, out_channel, self.scale, self.scale])(x)
...
class DRCT(keras.models.Model):
    def __init__(self,
        ...
        if in_chans == 3:
            ...
            self.mean = tf.constant(rgb_mean)
            self.mean = keras.ops.reshape(self.mean, [1, 1, 1, 3])
        else:
            self.mean = keras.ops.zeros(1, 1, 1, 1)
        ...
        for i_layer in range(self.num_layers):
            body_layer = RDG(dim=embed_dim, input_resolution=(patches_resolution[0], patches_resolution[1]),
                    ...
                    norm_layer=norm_layer, gc=gc, img_size=img_size, patch_size=patch_size) # x : [b, 4096,96] x_size : [64,64]
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_before_upsample = keras.Sequential(
                [keras.layers.Conv2D(num_feat, kernel_size=3, padding='same', name='conv_before_upsample'),  keras.layers.LeakyReLU(negative_slope=1e-2)])
